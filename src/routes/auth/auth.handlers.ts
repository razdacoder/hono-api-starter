...
export const register: AppRouteHandler<RegisterRoute> = async (c) => {
    ...
    const job = await defaultQueue.add(TASK.SendActivationEmail, { email: user.email, otp: otp });
    c.var.logger.info(`Job ${job.id} added to queue. Task scheduled for ${TASK.SendActivationEmail}`);
    return c.json(
      {
        success: true,
...
export const resendActivation: AppRouteHandler<ResendActivation> = async (c) => {
